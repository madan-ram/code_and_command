...
def infer_network(input, dropout_prob):
    ...
    network = max_pool_2d(network, 2)
    print network.get_shape(), 'CONV_F4 shape'
    # network = local_response_normalization(network)

    network = fully_connected(network, 4096, activation='relu', name='full1')
    network = dropout(network, dropout_prob)
    network = fully_connected(network, 1024, activation='relu', name='full2')
    network = dropout(network, dropout_prob)
    network = fully_connected(network, 128, activation='relu', name='full3')
    return network

def build_triplet_network(input_anchor, input_positive, input_negative, dropout_prob):
    # create anchore images
    with tf.variable_scope('TripletNetwork') as scope:
        anchore_network = infer_network(input_anchor, dropout_prob)
    
    # create network for positive images, with weights sharing
    with tf.variable_scope('TripletNetwork', reuse=True):
        positive_network = infer_network(input_positive, dropout_prob)

    # create network for negative images, with weights sharing
    with tf.variable_scope('TripletNetwork', reuse=True):
        negative_network = infer_network(input_negative, dropout_prob)

    return anchore_network, positive_network, negative_network


def diff_func(a, b):
    # tf.abs(tf.sub(x, y), name='abs_diff')
    return tf.sub(a, b, name='diff')

def build_binary_classifier_network(anchore_input_bin, test_input_bin, labels, dropout_prob_bin):
    diff_vector = diff_func(anchore_input_bin, test_input_bin)

    network = fully_connected(diff_vector, 128, activation='relu', name='full1_bin')
    network = dropout(network, dropout_prob_bin)
    network = fully_connected(network, 2, activation='relu', name='full2_bin')
